...
const MeetingCard = ({ meeting }: MeetingCardProps) => {
  const getRelation = () => {
    if (meeting.tenant) {
      return { icon: <User className="h-4 w-4 text-muted-foreground" />, name: meeting.tenant.name };
    }
    if (meeting.property) {
      return { icon: <Home className="h-4 w-4 text-muted-foreground" />, name: meeting.property.address };
    }
    if (meeting.owner) {
      return { icon: <Building2 className="h-4 w-4 text-muted-foreground" />, name: meeting.owner.name };
    }
    return null;
  };
  ...
  return (
    <Card className="mb-3 shadow-sm hover:shadow-md transition-shadow duration-200">
      <CardHeader className="p-4">
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <CardTitle className="text-base font-semibold text-foreground">{meeting.title}</CardTitle>
      ...
      <CardContent className="p-4 pt-0">
        ...
        {meeting.notes && (
          <p className="text-sm text-muted-foreground bg-muted p-2 rounded-md">{meeting.notes}</p>
        )}
      </CardContent>
    </Card>
  );
...
export const CalendarPage = () => {
  ...
  return (
    <MainLayout title={t('calendar:title')}>
      <PageContainer>
        <div className="h-full flex flex-col bg-muted p-2 sm:p-4">
          {/* Header */}
              ...
              <>
                ...
                <div className="md:hidden">
                  {meetingsForSelectedDay.length > 0 ? (
                    ...
                    <div className="text-center py-10 text-muted-foreground">{t('calendar:emptyState.noMeetings')}</div>
                  )}
                </div>
                ...
                <div className="hidden md:grid md:grid-cols-7 md:gap-2 h-full">
                  {weekDays.map(day => (
                    <div key={day.toISOString()} className="flex flex-col bg-card rounded-lg shadow-sm border border-border">
                      <div className={cn(
                          "text-center py-2 border-b border-border font-semibold",
                      ...
                      <div className="p-2 overflow-y-auto flex-1">
                        {getMeetingsForDay(day).length > 0 ? (
                          ...
                          <div className="text-xs text-center pt-4 text-muted-foreground/70">{t('calendar:emptyState.noMeetings')}</div>
                        )}
                      </div>
                    </div>
                  ))}
