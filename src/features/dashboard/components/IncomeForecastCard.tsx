          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div className="rounded-lg border p-3">
                <p className="text-slate-600">{t('dailyBrief.forecast.committedGross')}</p>
                <p className="font-semibold text-slate-900">{money(data?.committed_this_month_gross)}</p>
              </div>
              <div className="rounded-lg border p-3">
                <p className="text-slate-600">{t('dailyBrief.forecast.committedNet')}</p>
                <p className="font-semibold text-emerald-700">{money(data?.committed_this_month_net)}</p>
              </div>
              <div className="rounded-lg border p-3">
                <p className="text-slate-600">{t('dailyBrief.forecast.weightedGross')}</p>
                <p className="font-semibold text-slate-900">{money(data?.weighted_90_days_gross)}</p>
              </div>
              <div className="rounded-lg border p-3">
                <p className="text-slate-600">{t('dailyBrief.forecast.weightedNet')}</p>
