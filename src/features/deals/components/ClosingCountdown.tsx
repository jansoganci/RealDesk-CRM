...
export function ClosingCountdown({ closingDate, dealName }: ClosingCountdownProps) {
  ...
  let className = `${COLORS.success.bgLight} border-emerald-200 ${COLORS.success.textDark}`;
  ...
  if (daysRemaining <= 7 && daysRemaining > 0) {
    className = `${COLORS.warning.bgLight} border-amber-200 ${COLORS.warning.textDark}`;
    ...
    className = `${COLORS.danger.bgLight} border-red-200 ${COLORS.danger.textDark}`;
