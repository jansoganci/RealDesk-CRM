      ...
      <CardContent>
        <dl className="space-y-2">
          {rows.map(({ key, label }) => (
            <div
              key={key}
              className="flex flex-col gap-0.5 border-b border-gray-100 py-2 last:border-0 sm:flex-row sm:justify-between"
            >
              <dt className={`text-sm font-medium ${COLORS.muted.text}`}>{label}</dt>
              <dd className={`text-sm ${COLORS.gray.text900}`}>{formatDate(data[key])}</dd>
            </div>
          ))}
        </dl>
      </CardContent>
