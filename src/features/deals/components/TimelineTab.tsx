...
export function TimelineTab({ deal, readOnly = false }: TimelineTabProps) {
  ...
  return (
    <div className="space-y-4 relative">
      ...
      {loading && <p className={cn('text-sm', COLORS.gray.text600, 'dark:text-muted-foreground')}>{t('timeline.loading')}</p>}
                ...
                {group.rows.length === 0 ? (
                  <Card>
                    <CardContent className="py-4">
                      <p className={cn('text-sm', COLORS.gray.text600, 'dark:text-muted-foreground')}>{t('timeline.emptyPhase')}</p>
                    </CardContent>
                  </Card>
                ) : (
      ...
      {!loading && customMilestones.length > 0 && (
        <div className="space-y-3">
          <Card>
            <CardContent className="py-3">
              <p className={cn('text-sm font-semibold', COLORS.gray.text900, 'dark:text-foreground')}>{t('timeline.customSectionTitle')}</p>
            </CardContent>
          </Card>
