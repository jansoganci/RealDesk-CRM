...
`getEncryptionKey()` read a client-prefixed environment variable. Client-prefixed variables are inlined into the browser bundle — they are not server secrets and can be recovered from built assets. This key encrypted owner bank routing/account numbers and Tax IDs (consumed by `owners.service.ts`, `contractCreation.service.ts`, `contractUpdate.service.ts`). The AES-256-GCM implementation itself was correct — unique random IV per call, auth tag verified on decrypt — but a correct cipher with a globally-extractable static key gave no real confidentiality. Any authenticated user could pull an encrypted value from Supabase and decrypt it client-side with the same key every other browser received.
