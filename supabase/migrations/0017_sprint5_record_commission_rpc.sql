-- Sprint 5 T5: atomic record-commission RPC
-- Inserts commission + closes deal + updates property status in one transaction.
...
BEGIN
  ...
  SELECT d.org_id, d.deal_type
  INTO v_org_id, v_deal_type
  FROM public.deals d
  ...
  VALUES (
    ...
    v_property_id,
    v_deal_id,
    NULLIF(p_commission->>'contract_id', '')::uuid,
  ...
  SET
    ...
    actual_close_date = v_closing_date,
    updated_at = NOW()
  WHERE id = v_deal_id
    AND org_id = v_org_id
...
REVOKE ALL ON FUNCTION public.rpc_record_commission_and_close_deal(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_record_commission_and_close_deal(jsonb) TO authenticated;
