/*
  # Update create_contract_atomic RPC - Add commission_amount Support

  1. Changes
    - Add commission_amount column to contracts INSERT statement
    - Extract commission_amount from contract_data JSONB
    - Use NULLIF to convert 0 to NULL (triggers fallback to rent_amount)

  2. Logic
    - If commission_amount is provided (NOT NULL and > 0): Store the value
    - If commission_amount is NULL or 0: Store NULL (triggers fallback to rent_amount)
    - NULLIF(..., 0) converts 0 to NULL for consistency

  3. Backward Compatibility
    - Existing contracts have commission_amount = NULL
    - NULL values trigger fallback to rent_amount (existing behavior)
    - No breaking changes
