# RealDesk-CRM — .hermes/ Directory

This directory contains project-specific AI agent rules for RealDesk-CRM.

## Purpose

These files travel with the repo and define how AI agents (Hermes, Codex, Claude, Cursor, or any other tool) should behave when working on this project. They are source-of-truth project rules, not orchestration policy — orchestration policy lives in the Nexus V1 (multi-agent-orchestrator) skill.
...
| `hermes-state.json` | Runtime workflow state (gitignored) | Session start (if exists) |
| `plans/` | PLAN_ONLY plan output files | During planning |
| `tmp/` | Temporary working files | During active task |
